...
from orch_svc.hitl_copy import cta_hitl, mensagem_hitl_lead, validar_hitl_lead
...
# CTAs — cotação parametriza os "outros planos" (omite o já cotado).
_PLANOS_CATALOGO = (
    ("essencial", "Essencial"),
    ("completo", "Completo"),
    ("premium", "Premium"),
...
def cta_cotacao(*, plano_id: str | None = None, plano_nome: str | None = None) -> str:
    """CTA pós-cotação: detalhar coberturas OU comparar só com planos diferentes do atual."""
    atual = (plano_id or "").strip().lower()
    if not atual and plano_nome:
        atual = plano_nome.strip().lower()
    outros = [
        nome for pid, nome in _PLANOS_CATALOGO
        if pid != atual and nome.lower() != atual
    ]
    if not outros:
        alt = "outro plano"
    elif len(outros) == 1:
        ...
        alt = f"{outros[0]} ou {outros[1]}"
    return (
        f"Quer que eu detalhe as coberturas ou compare com {alt}?"
    )
...
def _spec_catalog() -> dict[str, FechamentoSpec]:
    ...
    for faixa in Faixa:
        ...
        specs[key] = FechamentoSpec(
            key=key,
            template="{corpo} {cta}",
            cta="{cta}",  # preenchido em resolver_fechamento
        )
    ...
    specs["escalar_humano"] = FechamentoSpec(
        key="escalar_humano",
        # Grau A: texto ao lead vem de hitl_copy (sem jargão 5xx/circuito).
        template="{hitl_lead}",
        cta=_CTA_HITL,
    )
    # Padrão das conversas ganho do dataset (boleto + apólice).
    specs["emitir_apolice"] = FechamentoSpec(
        key="emitir_apolice",
        template=(
            "Maravilha! Vou gerar o boleto e te mando a apólice do plano {plano} "
            "(R$ {premio}/mês). Bem-vindo!"
        ),
        cta="Bem-vindo!",
    )
...
def resolver_fechamento(
    ...
    *,
    ...
    # HITL grau A: lead nunca vê 503/circuito/retries — só cópia humana.
    if no.acao == "escalar_humano":
        motivos_raw = list(dec.motivos or ())
        params["motivos_internos"] = "; ".join(motivos_raw) or params.get("motivos", "")
        params["hitl_lead"] = mensagem_hitl_lead(motivos_raw)
        params["extra"] = ""
        params["cta"] = cta_hitl()
    else:
        params["extra"] = ""
    ...
    if no.acao == "apresentar_cotacao":
        cta = cta_cotacao(plano_id=no.plano_id, plano_nome=no.plano_nome)
        params["cta"] = cta
    elif no.acao == "escalar_humano":
        cta = cta_hitl()
        params["cta"] = cta
    else:
        cta = base.cta
    ...
    ) if no.acao in ("apresentar_cotacao", "emitir_apolice") else ()
...
def validar_fechamento_llm(
    ...
    *,
    ...
    if not (texto or "").strip():
        return False
    # Escalate: padrão A obrigatório (sem jargão, sem prêmio inventado).
    if spec.key == "escalar_humano":
        return validar_hitl_lead(texto)
    low = texto.lower()
    for ban in _CTA_PROIBIDAS:
        if ban in low:
            return False
    # cotou X → não relistar X como opção de "compare"
    plano = (params.get("plano") or "").strip().lower()
    if plano and "compare" in low:
        pos = low.find("compare")
        if pos >= 0 and plano in low[pos:]:
            return False
